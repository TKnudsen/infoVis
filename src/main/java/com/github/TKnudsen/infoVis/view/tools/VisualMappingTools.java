...
/**
 * <p>
 * Static helpers for validating and transforming data prior to visual
 * mapping: sanity-checking that world-to-number mappings produce non-null,
 * non-NaN values (1D/2D/n-D variants), computing min/max, and aggregating
 * relative-coordinate data into a 2D grid.
 * </p>
 *
 * @version 1.0
 */
public class VisualMappingTools {

	private VisualMappingTools() {
	...
	public static <D> List<D> sanityCheckFilter(Iterable<D> data,
		...
		if (data == null) {
			if (warnForQualityLeaks)
				System.err.println("VisualMappingTools.sanityCheckFilter: given data was null");
			return returnList;
		}

		if (worldToDoubleMapping == null) {
			if (warnForQualityLeaks)
				System.err.println("VisualMappingTools.sanityCheckFilter: given worldToDoubleMapping was null");
			return returnList;
		}
		...
		try {
			for (D t : data) {
				...
				if (d != null && !Double.isNaN(d.doubleValue()))
					...
					System.err.println("VisualMappingTools.sanityCheckFilter: object " + t
	...
	public static <D> List<D>[][] visualAggregation2D(Iterable<D> data,
		...
		if (data == null) {
			if (warnForQualityLeaks)
				System.err.println("VisualMappingTools.visualAggregation2D: given data was null");
			return null;
		}

		if (worldToDoubleMappingX == null) {
			if (warnForQualityLeaks)
				System.err.println("VisualMappingTools.visualAggregation2D: given worldToDoubleMappingX was null");
			return null;
		}

		if (worldToDoubleMappingY == null) {
			if (warnForQualityLeaks)
				System.err.println("VisualMappingTools.visualAggregation2D: given worldToDoubleMappingY was null");
			return null;
		}
		...
		if (x < 1 || y < 1) {
			if (warnForQualityLeaks)
				System.err
						.println("VisualMappingTools.visualAggregation2D: given aggregation count < 1: " + x + " and " + y);
			return null;
		}
				...
				if (xV != null && !Double.isNaN(xV.doubleValue())) {
					...
					if (yV != null && !Double.isNaN(yV.doubleValue())) {
						...
						if (xV.doubleValue() < 0.0 || xV.doubleValue() > 1.0) {
							if (warnForQualityLeaks)
								System.err.println("VisualMappingTools.visualAggregation2D: given value for " + t
										+ " was not relative: " + xV);
							return null;
						}
