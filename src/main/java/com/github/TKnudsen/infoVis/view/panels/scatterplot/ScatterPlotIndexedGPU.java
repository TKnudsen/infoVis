 ...
 * @since 2025
 */
public class ScatterPlotIndexedGPU<T> extends XYNumericalChartPanel<Double, Double>
	...
	// Scratch buffer: JOGL's broken FBO->Swing blit is redirected here and
	// discarded.
	private java.awt.image.BufferedImage glBlitScratch = null;
	...
	public ScatterPlotIndexedGPU(List<T> data, Function<? super T, ? extends Paint> colorMapping,
		...
		glCanvas = createGLPanel(scatterPlotPainter);
		add(glCanvas);
	 ...
	 * {@link ScatterPlotIndexedGPUPainter} that is immediately discarded when this
	 * method's own assignment overwrites it.
	 *
	 * @param data the data to plot
	 */
	protected void initializePainter(List<T> data) {
		this.scatterPlotPainter = new ScatterPlotIndexedGPUPainter<T>(data, colorMapping, worldPositionMappingX,
	...
	private GLJPanel createGLPanel(ScatterPlotIndexedGPUPainter<T> painter) {
...
//		GLCapabilities caps = new GLCapabilities(profile);
		...
		// paint of the GLJPanel is a no-op. This prevents the failed FBO->Swing blit
		// from painting black over the glReadPixels image we draw in paintComponent().
		...
		panel.addGLEventListener(new GLEventListener() {
			...
			public void reshape(GLAutoDrawable d, int x, int y, int w, int h) {
				painter.reshapeGL(w, h);
				// GL surface has been resized -- schedule a fresh render at the new size.
				javax.swing.SwingUtilities.invokeLater(() -> ScatterPlotIndexedGPU.this.repaint());
			}
			...
			public void display(GLAutoDrawable d) {
				...
				painter.displayGL(d, currentChartRectangle);
			}
		});
...
//			System.out.println("Chart rectangle: x=" + x + ", y=" + y + ", w=" + w + ", h=" + h);
//			System.out.println("Panel size: " + getWidth() + "x" + getHeight());
	...
	protected void paintComponent(Graphics g) {
		...
		if (glCanvas != null && scatterPlotPainter.isGpuRendererInitialized()) {
			java.awt.image.BufferedImage frame = scatterPlotPainter.getLastRenderedFrame();
	...
	void doGLRender() {
		...
		if (glCanvas != null && scatterPlotPainter.isGpuRendererInitialized()) {
			...
			// displayGL()
			// -> readbackToImage() -> lastRenderedFrame updated.
			// The blit inside super.paintComponent is redirected to glBlitScratch
			// (discarded).
			glCanvas.display();
	...
	private void forwardMouseEventsFromGLCanvas() {
		MouseAdapter forwarder = new MouseAdapter() {
			private MouseEvent translateEvent(MouseEvent e) {
				...
				return new MouseEvent(ScatterPlotIndexedGPU.this, e.getID(), e.getWhen(), e.getModifiersEx(),
			...
			public void mousePressed(MouseEvent e) {
				...
				ScatterPlotIndexedGPU.this.repaint(); // Force repaint
			}
			...
			public void mouseDragged(MouseEvent e) {
				...
				ScatterPlotIndexedGPU.this.doGLRender();
			}
			...
			public void mouseWheelMoved(java.awt.event.MouseWheelEvent e) {
				...
				java.awt.event.MouseWheelEvent translated = new java.awt.event.MouseWheelEvent(
						ScatterPlotIndexedGPU.this, e.getID(), e.getWhen(), e.getModifiersEx(),
						e.getX() + glCanvasPos.x, e.getY() + glCanvasPos.y, e.getClickCount(), e.isPopupTrigger(),
