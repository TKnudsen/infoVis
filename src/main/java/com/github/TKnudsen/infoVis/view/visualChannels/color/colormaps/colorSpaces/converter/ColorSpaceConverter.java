...
/**
 * <p>
 * Converts colors/images between color spaces (RGB, HSB, LAB, XYZ, xyY).
 * Adapted from ImageJ 1.x's own core class
 * {@code ij.process.ColorSpaceConverter}
 * (https://github.com/imagej/ImageJ/blob/master/ij/process/ColorSpaceConverter.java);
 * not this project's own original work.
 * </p>
 *
 * <p>
 * ImageJ 1.x is developed at the U.S. National Institutes of Health by an
 * employee of the Federal Government in the course of his official duties, and
 * is therefore, pursuant to Title 17, Section 105 of the United States Code,
 * not subject to copyright protection and in the public domain
 * (https://imagej.net/ij/source/LICENSE.txt). No attribution is legally
 * required; the origin is recorded here for provenance.
 * </p>
 *
 * @author Duane Schwartzwald, Harry Parker
 * @since 2004
 */
public class ColorSpaceConverter {
	 ...
	 */
	private static final double[] chromaD50 = { 0.3457, 0.3585, 100.0 };
	private static final double[] chromaD55 = { 0.3324, 0.3474, 100.0 };
	private static final double[] chromaD65 = { 0.3127, 0.3290, 100.0 };
	private static final double[] chromaD75 = { 0.2990, 0.3149, 100.0 };
	...
	public ColorSpaceConverter() {
		whitePoint = D65.clone();
		chromaWhitePoint = chromaD65.clone();
	}
	...
	public ColorSpaceConverter(String white) {
		...
		if (white.equalsIgnoreCase("d50")) {
			whitePoint = D50.clone();
			chromaWhitePoint = chromaD50.clone();
		} else if (white.equalsIgnoreCase("d55")) {
			whitePoint = D55.clone();
			chromaWhitePoint = chromaD55.clone();
		} else if (white.equalsIgnoreCase("d65")) {
			whitePoint = D65.clone();
			chromaWhitePoint = chromaD65.clone();
		} else if (white.equalsIgnoreCase("d75")) {
			whitePoint = D75.clone();
			chromaWhitePoint = chromaD75.clone();
		}
	}

	public static double[] getD50() {
		return D50.clone();
	}

	public static double[] getD55() {
		return D55.clone();
	}

	public static double[] getD65() {
		return D65.clone();
	}

	public static double[] getD75() {
		return D75.clone();
	}
	...
	public static double[] getChromaD50() {
		return chromaD50.clone();
	}

	public static double[] getChromaD55() {
		return chromaD55.clone();
	}

	public static double[] getChromaD65() {
		return chromaD65.clone();
	}

	public static double[] getChromaD75() {
		return chromaD75.clone();
	}

	public double[] getChromaWhitePoint() {
		return chromaWhitePoint.clone();
	}
	...
	public int[] XYZtoRGB(double X, double Y, double Z) {
		...
		double m1 = Math.max(r, g);
		double m2 = Math.max(m1, b);
		if (m2 > 1) {
			r /= m2;
			g /= m2;
			b /= m2;
